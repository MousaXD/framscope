...
BUILD_SCRIPT="$ROOT/scripts/build-ffmpeg-android-hwdecode-probe.sh"
...
WORK_DIR="$ROOT/.native/work/ffmpeg-${FFMPEG_VERSION}-${ABI}-hwdecode-probe"
...
for tool in curl sha256sum tar make; do
  command -v "$tool" >/dev/null || {
    echo "error: $tool is required to build the FFmpeg MediaCodec probe" >&2
    exit 1
  }
done
...
if [[ -f "$MARKER" ]] \
  ...
  && grep -qx "FRAMESCOPE_PROBE=android-mediacodec" "$MARKER" \
  && [[ -f "$PREFIX/include/libavcodec/avcodec.h" ]] \
  && [[ -f "$PREFIX/lib/libavcodec.a" ]] \
  && [[ -f "$PREFIX/lib/libavformat.a" ]] \
  && [[ -f "$PREFIX/lib/libavutil.a" ]]; then
  echo "Reusing FFmpeg MediaCodec probe prefix: $PREFIX"
  exit 0
fi
...
mkdir -p "$PREFIX/share/framescope"
{
  ...
  echo "FFMPEG_BUILD_SCRIPT_SHA256=$BUILD_SCRIPT_SHA256"
  echo "ANDROID_ABI=$ABI"
  echo "ANDROID_ARCH=$ARCH"
  echo "ANDROID_API=$ANDROID_API"
  echo "NDK_VERSION=$NDK_VERSION"
  echo "FRAMESCOPE_PROBE=android-mediacodec"
  echo "FFMPEG_DECODERS=h264,hevc,vp9,av1,h264_mediacodec,hevc_mediacodec,vp9_mediacodec,av1_mediacodec"
} > "$MARKER"
...
echo "FFmpeg MediaCodec probe prefix ready: $PREFIX"
