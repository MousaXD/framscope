use crate::{HybridFrameGrouper, HybridGroupingError};
...
/// Cooperative cancellation for streaming grouping work.
///
/// The token is cheap to clone and is checked before each frame is compared. A cancelled push does
/// not mutate grouping state, allowing callers to discard the incomplete derived result safely.
...
/// Cancellation-aware, instance-instrumented wrapper around the bounded hybrid grouper.
///
/// This wrapper never accumulates input frames or completed groups. Callers consume each completed
/// group as it is returned, keeping memory independent of video duration.
#[derive(Debug, Clone)]
...
mod tests {
    ...
    #[test]
    fn cancellation_before_push_does_not_mutate_grouping_state() {
    ...
    #[test]
    fn long_stream_is_consumed_incrementally_without_collecting_frames_or_groups() {
