...
fn open_session(
    ...
    let mut sessions = microscope_sessions()
        .lock()
        .map_err(|_| MicroscopeFailure::new("bridge_error", "microscope session state is poisoned"))?;
...
fn ensure_session_capacity() -> Result<(), MicroscopeFailure> {
    let sessions = microscope_sessions()
        .lock()
        .map_err(|_| MicroscopeFailure::new("bridge_error", "microscope session state is poisoned"))?;
    if sessions.len() >= MAX_MICROSCOPE_SESSIONS {
        Err(MicroscopeFailure::new(
...
fn jump_to_timestamp(
    ...
    with_session_mut(session_id, |session| {
        let entry = match selection {
            0 => session.index.frame_at_or_before_us(timestamp_us).map_err(from_index)?,
            1 => session.index.frame_at_or_after_us(timestamp_us).map_err(from_index)?,
            2 => choose_nearest_us(
                timestamp_us,
                session.index.frame_at_or_before_us(timestamp_us).map_err(from_index)?,
                session.index.frame_at_or_after_us(timestamp_us).map_err(from_index)?,
            )?,
            _ => {
                return Err(MicroscopeFailure::new(
                    "invalid_request",
...
fn stepped_frame(current: FrameId, frame_count: u64, delta: i32) -> Result<FrameId, MicroscopeFailure> {
    match delta {
        -1 if current.0 > 0 => Ok(FrameId(current.0 - 1)),
        1 if current.0.checked_add(1).is_some_and(|next| next < frame_count) => {
    ...
    match (before, after) {
        ...
        (Some(before), Some(after)) => {
            let before_us = before.timestamp_us().ok_or_else(|| {
                MicroscopeFailure::new("index_error", "indexed timestamp cannot be converted to microseconds")
            })?;
